...
EN ;ENTER NEW PAT IN FILE 442;
 ...
 S PRCHP("S")=+$G(PRCHP("S"))
 ;
ENPO S DIC="^PRC(442.6,",DIC(0)="QEMZ",DIC("S")="I +$P(^(0),U,1)=PRC(""SITE""),+$P(^(0),U,5)=PRCHP(""S"")" S:$D(PRCHP("S2")) DIC("S")=DIC("S")_PRCHP("S2")
 W !!,"ENTER A NEW ",PRCHP("A")," OR A COMMON NUMBERING SERIES"
 W !?3,PRCHP("A"),": " R X:DTIME G:X=""!(X=U) ENPOQ I $E(X,1)="?" S D="C" D IX^DIC G ENPO
 ;
