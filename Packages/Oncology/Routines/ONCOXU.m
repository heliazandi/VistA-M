ONCOXU ; GENERATED FROM 'ONCO XABSTRACT RECORD' PRINT TEMPLATE (#826) ; 03/14/16 ; (FILE 165.5, MARGIN=80)
 G BEGIN
N W !
...
A1 ;
 ...
 S X=$G(^ONCO(160,D0,0)) D N:$X>8 Q:'DN  W ?8 S Y=$P(X,U,8) W:Y]"" $E($$SET^DIQ(160,10,Y),1,25)
 ...
 S X=$G(^ONCO(160,D0,1)) D N:$X>47 Q:'DN  W ?47 S Y=$P(X,U,1) W:Y]"" $E($$SET^DIQ(160,15,Y),1,5)
 W ?47 W !," " K DIP K:DN Y
...
A1R ;
 ...
 S X=$G(^ONCO(165.5,D0,7)) S Y=$P(X,U,2) W:Y]"" $E($$SET^DIQ(165.5,91,Y),1,14)
