ONCOXCL ; GENERATED FROM 'ONCO XCONTACT LIST' PRINT TEMPLATE (#1325) ; 07/18/96 ; (FILE 160, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1325,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
