DIFROMS2 ;SFISC/DCL/TKW - INSTALL DD FROM SOURCE ARRAY ;12APR2016
...
DIFRFIA I '$D(DIFRFIA) N DIFROMS2 S DIFRFIA="DIFROMS2"
 F DIFRFILE=0:0 S DIFRFILE=$O(@DIFRSA@("^DIC",DIFRFILE)) Q:'DIFRFILE  D
 .S:'$D(@DIFRFIA@(DIFRFILE)) @DIFRFIA@(DIFRFILE)=@DIFRSA@("^DIC",DIFRFILE,DIFRFILE,0) ;NAME OF FILE
 .S:'$D(@DIFRFIA@(DIFRFILE,0)) @DIFRFIA@(DIFRFILE,0)=@DIFRSA@("^DIC",DIFRFILE,DIFRFILE,0,"GL") ;GLOBAL LOCATION OF FILE
 .S @DIFRFIA@(DIFRFILE,0,0)=DIFRFILE ;NUMBER OF FILE
...
FILE ;
 ...
 S DIFR01=$G(@DIFRFIA@(DIFRFILE,0,1),"y^y^f") ;UPDATE DATA DICTIONARY [1S] ^ (#222.2) SEND SECURITY CODE [2S] ^ (#222.3) SEND FULL
...
CHKDDNUM .S DIFRX=$O(^DIC(DIFRFILE)) I DIFRD<DIFRFILE!(DIFRD'<DIFRX&DIFRX) D ERR(3) Q  ;DON'T ALLOW SETTING OF WRONG ^DD!
...
PARTIAL I 'DIFRFDD D  ;PARTIAL DEFINITION
 ...
 ..D K12 M ^DD(DIFRD)=@DIFRSA@("^DD",DIFRFILE,DIFRD) ;HERE IS WHERE A WHOLE DD COMES OVER!
...
DIKZ I $D(^DD(DIFRFILE,0,"DIK")) D
 ...
 ;
K12 N DD,D
 F DD=0:0 S DD=$O(@DIFRSA@("^DD",DIFRFILE,DD)) Q:'DD  D
