VEXRX    ;MUSKOGEE VAMC/GLD - AUDIOFAX SUBROUTINE - 6-21-94
 ;;7.0;OUTPATIENT PHARMACY;**197**;JUN 1994
...
VEX3 K PSOBBC("IRXN"),VEXXFLAG F  S VEXPTRX=$O(^VEXHRX(19080,PSOINST,VEXPTRX))  D  Q:VEXANS="N"!($G(VEXXFLAG))
 ...
 . I '$D(^PSRX(+$P(VEXPTRX,"-",2),0)),VEX648=1 D VEX5,VEX12 Q  ;SKIPS ERRONEOUS ENTRIES
...
VEX5 ;
 ...
 S:RENFLG=0 ^VEXHRX(19080,PSOINST,VEXPTRX)=DT        ;MARKS NODE AS PROCESSED
 ;S ^VEXHRX(19080,PSOINST,VEXPTRX)=DT ;MARKS NODE AS PROCESSED
 I $D(PSOBBC("DFLG")),PSOBBC("DFLG")=1 D VEX12 ;FLAGS UNSUCCESSFUL ATTEMPTS TO REFILL.
 ; @3
...
END D PROCESSX^PSOBBC
 ...
 ; @3
  ;I CNT1>10 D MLMSG^APUVEX  ; PVMAC/BFD 8-10-04 Was chking RENFLG but not work so chg to CNT1 - Send mail msg to renew mail group - flg so not err site not renewal
 I CNT1>10 D MLMSG^APUVEX2  ; PVMAC/BFD 8-10-04 Was chking RENFLG but not work so chg to CNT1 - Send mail msg to renew mail group - flg so not err site not renewal
 ; I CNT1>10!(RFY>0)!(NRF>0) D MMKTR^APUVEX(RFY,NRF) ; PVMAC/BFD 8-27-04 Sent ktr msg to 'mgr' mail group
 I CNT1>10!(RFY>0)!(NRF>0) D MMKTR^APUVEX2(RFY,NRF) ; PVMAC/BFD 8-27-04 Sent ktr msg to 'mgr' mail group
...
RENEWCHK ; Checks ^VEXHRX node for renewal information
 ...
 . ;D RENEWAL^APUVEX(PATIEN,RXNUM,RENEW,PROVP,USR,CNT1,.RESULT)
 . D RENEWAL^APUVEX1(PATIEN,RXNUM,RENEW,PROVP,USR,CNT1,.RESULT)
 . S RENFLG=1
 . S $P(^VEXHRX(19080,PSOINST,VEXPTRX),"^")=DT
 . S $P(^VEXHRX(19080,PSOINST,VEXPTRX),"^",6)=RESULT
 . I RESULT=0 S CNT1=CNT1+1
 . I RESULT=1 S CNT1=CNT1+1
 . Q
 Q
